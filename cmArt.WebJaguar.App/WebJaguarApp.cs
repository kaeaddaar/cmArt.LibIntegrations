...
+ "being sent to Shopify");
            ...
            logger.LogInformation("Begin converting Assembled Inventory Records to the Common Field Format for WebJaguar.");
            IEnumerable<S5InventoryLogic> adapters = ECommInvAss.Select(Inv =>
            {
                S5InventoryLogic tmp = new S5InventoryLogic();
                tmp.init(Inv);
                return tmp;
            });
            // evaluate converting to POCO
            IEnumerable<WJCommonFields> wJCommonFields = adapters.Select(adapter =>
            {
                WJCommonFields tmp = new WJCommonFields();
                tmp.CopyFrom(adapter);
                return tmp;
            });

            #region // Use facade to create data load format from Assembled Inventory Data
            ...
            #endregion // Use facade to create data load format from Assembled Inventory Data

            Func<IWJCommonFields, IWJCommonFields, bool> fEquals = (from, to) => { return from.Equals(to); };

            #region fEquals and variants
            ...
            #endregion fEquals and variants

            // ----- Get data from previous sync, and determine changed products then adds -----

            #region Direct from Shopify
            //if (false)
            //{
